    ...
    def face_recognition(self):
        def draw(img, classifier, scalefactor, minneighbors, color, text, clf):
            ...
            for (x, y, w, h) in feature:
                ...
                my_curse.execute("SELECT student_name FROM student WHERE student_id=" + str(id))
                feach = my_curse.fetchone()
                feach = "+".join(feach)

                my_curse.execute("SELECT course FROM student WHERE student_id=" + str(id))
                feach1 = my_curse.fetchone()
                feach1 = "+".join(feach1)
                    
                if confidence > 77:
                    cv2.putText(img, f"Name: {feach}", (x, y-55), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 4)
                    cv2.putText(img, f"Course: {feach1}", (x, y-30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 4)
                else:
        ...
        clf.read("trainnerdata_student.xml")
